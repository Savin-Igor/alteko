...
function VotingContent() {
  ...
  async function handleCreateCampaign(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        setMessage({ type: 'success', text: `Голосование создано. Ссылка для жильцов: /voting/${data.campaignId}` })
        setCampaigns((prev) => [
          ...prev,
  ...
  const STATUS_LABEL: Record<string, string> = {
    DRAFT: 'Черновик', ACTIVE: 'Активно', COMPLETED: 'Завершено', CANCELLED: 'Отменено',
  }
      ...
      <main className="flex-1 px-4 py-6 max-w-2xl mx-auto w-full space-y-5">
        ...
        <div className="card space-y-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              CSV-файл: квартира;площадь_м2;доля_%;zemesgramata_ref (опционально)
            </p>
          </div>
          ...
          <form onSubmit={handleUploadOwners} className="space-y-3">
              ...
              <div className={`w-full border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-colors ${
                ...
                {csvFile ? (
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{(csvFile.size / 1024).toFixed(0)} КБ</p>
                  </div>
                ) : (
                  <div>
                    <p className="text-sm text-gray-600">Выберите CSV-файл</p>
                    <p className="text-xs text-gray-400 mt-1">Получить из Zemesgrāmata</p>
                  </div>
                )}
              </div>
            </label>
            ...
            >
              ...
              ) : 'Загрузить список собственников'}
            </button>
          </form>
          ...
          <p className="text-xs text-gray-400">
            ...
            {' '} или ручной CSV от правления
          </p>
        </div>

        {/* Step 2: Create campaign */}
        <div className="card space-y-4">
          <h2 className="font-medium text-gray-800">Шаг 2. Создать кампанию</h2>
          <form onSubmit={handleCreateCampaign} className="space-y-3">
            <div>
              ...
              <input
                ...
                placeholder="Реновация здания 2025 года"
            ...
            <div>
              <label htmlFor="vote-deadline" className="text-sm text-gray-600 block mb-1">
                Срок голосования (необязательно)
              </label>
              <input
                id="vote-deadline"
                type="datetime-local"
          ...
          <p className="text-xs text-gray-500">
            После создания жильцы получают ссылку для голосования через Smart-ID или eParaksts.
            Требуется ≥50% голосов «за».
          </p>
        </div>
        ...
        {campaigns.length > 0 && (
          <div className="space-y-3">
            <h2 className="font-medium text-gray-800">Кампании</h2>
            {campaigns.map((c) => (
              <div key={c.id} className="card flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    ...
                    {' · '}
                    За: {Math.round(c.currentYesShare * 100)}%
                  </p>
                </div>
                <Link
                  href={`/voting/${c.id}`}
